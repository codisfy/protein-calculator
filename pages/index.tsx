...
const Home: NextPage = () => {
  ...
  // check if user is loading a saved session
  useEffect(() => {
    const getSession = async () => {
      try {
        await Firestore.anonymousLogin()
        // not on saved page and has something in local storage
        if (!id && !!localStorage.getItem('session-id')) {
          router.push(`/saved/${localStorage.getItem('session-id')}`);
          return
        } else if (!id) {
          setDefaultData()
          return
        ...
        if (data.length) {
          ...
          setDefaultData()
          router.push("/") // redirect to home page as session not found
        }
      } catch (error) {
        setDefaultData()
      }
  ...
  // save existing session or create a new one and redirect user
